...
def filter_bank_real(M, N, J, L, addhaar=False):
    """
    ...
    if addhaar==True:
        # add Haar filter without multires
        filters['psi0'] = haar_2d(M,N)
        
    return filters

def haar_2d(M,N):
    # return fft2 of the haar 2d
    # each filter size (M,N,2)
    hatpsi={} 
    psi1 = np.zeros((M,N), np.complex64)
    psi1[0,0]=1/4
    psi1[0,1]=-1/4
    psi1[1,0]=1/4
    psi1[1,1]=-1/4
    hatpsi1 = convert_complexNp_floatT(fft2(psi1))
    hatpsi[1]=hatpsi1
    psi2 = np.zeros((M,N), np.complex64)
    psi2[0,0]=1/4
    psi2[0,1]=1/4
    psi2[1,0]=-1/4
    psi2[1,1]=-1/4
    hatpsi2 = convert_complexNp_floatT(fft2(psi2))
    hatpsi[2]=hatpsi2
    psi3 = np.zeros((M,N), np.complex64)
    psi3[0,0]=1/4
    psi3[0,1]=-1/4
    psi3[1,0]=-1/4
    psi3[1,1]=1/4
    hatpsi3 = convert_complexNp_floatT(fft2(psi3))
    hatpsi[3]=hatpsi3
    return hatpsi

# (M,N) -> (M,N,2)
def convert_complexNp_floatT(psi):
    assert(len(psi.shape)==2)
    return torch.FloatTensor(np.stack((np.real(psi),np.imag(psi)),axis=2))

def filter_bank(M, N, J, L, addhaar=False, cache=False):
    """
        Builds in Fourier the Morlet filters used for the scattering transform.
        Each single filter is provided as a dictionary with the following keys:
        * 'j' : scale
    ...
    """
    if not cache:
        return filter_bank_real(M, N, J, L, addhaar)
    else:
