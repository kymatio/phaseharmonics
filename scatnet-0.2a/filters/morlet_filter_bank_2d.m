...
function filters = morlet_filter_bank_2d(size_in, options)
	...
	for j = 0:J-1
		for theta = 1:numel(angles)			
			angle = angles(theta);
			scale = 2^(j/Q - res);
            cond = 0; % ~(j==J-1 && theta==2);
            if cond
                psi.filter{p} = morlet_2d_noDC_F(N, ...
                    M,...
                    sigma_psi*scale,...
                    slant_psi,...
                    xi_psi/scale,...
...
%                 psi.filter{p}(1,1)
%                 figure
%                 imagesc(imag(ifftshift(ifft2(psi.filter{p}))))
%                 title(sprintf('Im \\psi, j=%d, \\theta=%d',...
%                     j+1,theta),'FontSize',20)
            else
%                 offset=[0 0];
...
%                 end
                ...
                if (j==J-1 && theta==2)
                    imagesc(real(ifft2(psi.filter{p})))
                end
            end
...
%                 title(sprintf('Re[\\psi_%d]',j+1),...
%                         'FontSize',18)
%                 subplot(1,3,2)
%                 imagesc(real(fftshift(filter_spatial))); 
%                 colorbar; axis square
%                 title(sprintf('fftshift[Re[\\psi_%d]]',j+1),...
%                         'FontSize',18)
%                 subplot(1,3,3)
                %imagesc(real(fft2(filter_spatial)))
%                 imagesc(psi.filter{p})
%                 psi.filter{p}(2:end,1)=0;
%                 psi.filter{p}(1,2:end)=0;
%                  plot(psi.filter{p}(1,:))
%                 imagesc(real(fftshift(ifft2(psi.filter{p}))))
%                 colorbar; axis square
                %title(...
                %    sprintf('Re[IFFT2[Re[FFT2[\\psi_%d]]]]',j+1),...
                %    'FontSize',18)
%             end
