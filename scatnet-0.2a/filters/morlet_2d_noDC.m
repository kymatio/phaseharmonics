...
%     % make sure fft2 being real
%     gaussian_envelope(1,2:end) = ...
%         (gaussian_envelope(1,2:end)+gaussian_envelope(1,end:-1:2))/2;
%     gaussian_envelope(2:end,1) = ...
%         (gaussian_envelope(2:end,1)+gaussian_envelope(end:-1:2,1))/2;
%     
	oscilating_part = gaussian_envelope .* exp(1i*(x*xi*cos(theta) + y*xi*sin(theta)));
	
%     % make sure fft2 being real
%     oscilating_part(1,2:end) = ...
%         (oscilating_part(1,2:end)+conj(oscilating_part(1,end:-1:2)))/2;
%     oscilating_part(2:end,1) = ...
%         (oscilating_part(2:end,1)+conj(oscilating_part(end:-1:2,1)))/2;
%     oscilating_part(2:end,2:end) = ...
%         (oscilating_part(2:end,2:end)+conj(oscilating_part(end:-1:2,end:-1:2)))/2;
%     
